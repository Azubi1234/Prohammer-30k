...
def base_count_from_source(txt):
    ...
    # stop at the next standard heading
...
# Emperor's Children entries lost their Source Entry text in Rev52, so use their final-draft base sizes.
BASE_OVERRIDES={
...
for unit in list(top):
    ...
    mx=max_constraint(add)
    pc=points(add); up=points(unit)
    ...
    extra=int(float(mx.get('value','0'))); per=float(pc.get('value','0')); base_cost=float(up.get('value','0'))
    total=base+extra
    # Convert the old "additional models" counter into a true total squad-size counter.
    add.set('name','Squad Models'); add.set('defaultAmount',str(base))
    mx.set('value',str(total))
    ...
    # Preserve exact old minimum-squad cost while allowing the same per-model increment above minimum.
    up.set('value',str(base_cost - base*per))
    # The old explanatory "Additional model" rule is now redundant.
    rs=add.find(C('rules'))
    if rs is not None:
        for r in list(rs):
...
# Fix the EC Sonic Weaponry HSS with explicit weapon choices rather than inherited HSS references.
...
if sonic is not None:
    # Remove any previous Rev53 group on rerun.
    ...
    note('WARNING: Sonic Weaponry Heavy Support Squad not found.')
...
# Validation, allowing legal CAT<->GST references both ways.
