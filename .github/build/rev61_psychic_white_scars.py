...
def group(e, id_, name, minv=None, maxv=None):
    ...
    g = ET.SubElement(gs, C("selectionEntryGroup"), {
        "id": id_, "name": name, "hidden": "false", "collective": "false", "import": "true"
    })
    if minv is not None:
...
def option(g, id_, name, cost=0, maxv=1):
    ...
    s = ET.SubElement(ses, C("selectionEntry"), {
        "id": id_, "name": name, "type": "upgrade", "hidden": "false", "import": "true"
    })
    add_constraint(s, id_ + "-max", "max", maxv)
    ET.SubElement(ensure(s, "costs"), C("cost"), {
        "name": "Points", "typeId": "pts", "value": str(cost)
    })
    return s

def add_profile(e, id_, name, range_, strength, ap, type_):
    p = ET.SubElement(ensure(e, "profiles"), C("profile"), {
        "id": id_, "name": name, "hidden": "false", "typeId": "prof-ranged", "typeName": "Ranged Weapon"
    })
...
def add_conditional_modifier(e, id_, field, value, child_id, scope="root-entry"):
    m = ET.SubElement(ensure(e, "modifiers"), C("modifier"), {
        "id": id_, "type": "set", "value": str(value), "field": field
    })
    cs = ET.SubElement(m, C("conditions"))
    ET.SubElement(cs, C("condition"), {
        "type": "atLeast", "value": "1", "field": "selections", "scope": scope,
        "childId": child_id, "shared": "true", "includeChildSelections": "true", "includeChildForces": "false"
    })
    return m

def rewrite_ids(node, prefix):
    mapping = {x.get("id"): prefix + x.get("id") for x in node.iter() if x.get("id")}
    for x in node.iter():
        old = x.get("id")
        if old in mapping:
            x.set("id", mapping[old])
        for attr in ("targetId", "childId", "field"):
            value = x.get(attr)
            if value in mapping:
                x.set(attr, mapping[value])
...
veteran_transport = by_id(cr, "veteran-transport")

for x, msg in [
    (cent, "Centurion"), (lib, "Librarian"), (storm, "Stormseer"),
    (storm_ml2, "Stormseer Epistolary"), (dark, "Dark Sons"),
    (dark_transport, "Dark Sons transport"), (dark_jump, "Dark Sons Jump Packs"),
    (veteran_transport, "Veteran transport template"),
]:
    assert x is not None, msg

# Stormseer: keep the upgrade entirely inside the Centurion/Consul block and give
# its fixed shooting power a real ranged profile.
remove_rule_named(storm, "Stormseer Consul")
remove_rule_named(storm, "Unseen Bolt")
add_rule(
    storm, "r61-ws-stormseer-rule", "Stormseer Consul",
    "Replace the Centurion's Chainsword with a Force Weapon. Gains Psyker (Mastery Level 1), "
    "Adamantium Will and Legion Support Officer. A White Scars army uses the Stormseer Consul "
    "instead of the normal Legion Librarian Consul."
)
add_profile(
    storm, "r61-ws-unseen-bolt-profile", "Unseen Bolt", '36"', "6", "4",
    "Assault 1, Blast, Pinning, Psychic"
)
add_rule(
    storm, "r61-ws-unseen-bolt-rule", "Unseen Bolt",
    "Psychic Shooting Power. Use during the Stormseer's Shooting phase instead of firing another weapon. "
    "Take a Psychic Test; if successful resolve the attack using the profile shown. Unseen Bolt may not "
    "be invoked during Return Fire, Overwatch or Stand & Shoot."
)
add_power_choice_group(storm_ml2, "r61-ws-stormseer-power2", "Additional Psychic Power — select 1")

# Generic Librarian: ML1 chooses one general ProHammer power; Epistolary ML2 chooses a second.
...
if epi is None:
    epi = option(
        group(lib, "r61-librarian-epistolary-group", "Librarian Options", None, 1),
        "r61-librarian-epistolary", "Epistolary — Mastery Level 2", 25, 1
    )
...
# Rebuild Dark Sons dedicated transport choices from exact known transport IDs.
# Rev60 used substring lookup, which accidentally selected Damocles Command Rhino
# and the Heavy Support Land Raider Battle Squadron.
wipe(dark_transport, "entryLinks")
wipe(dark_transport, "selectionEntryGroups")

vlinks = veteran_transport.find(C("entryLinks"))
assert vlinks is not None
wanted_targets = ["transport-rhino", "transport-drop-pod", "transport-dreadclaw"]
dest_links = ensure(dark_transport, "entryLinks")
for target in wanted_targets:
    src = next((l for l in vlinks.findall(C("entryLink")) if l.get("targetId") == target), None)
    assert src is not None, target
    clone = deepcopy(src)
    rewrite_ids(clone, "r61-ws-dark-")
    dest_links.append(clone)

# Plain "Land Raider" follows the project's existing dedicated-transport representation:
# choose one standard pattern (Phobos, Proteus or Achilles), not the Heavy Support squadron.
v_nested = veteran_transport.find(C("selectionEntryGroups"))
lr_src = None
if v_nested is not None:
    lr_src = next((g for g in v_nested.findall(C("selectionEntryGroup")) if "land raider dedicated transport" in (g.get("name") or "").lower()), None)
assert lr_src is not None, "Land Raider dedicated-transport template not found"
lr = deepcopy(lr_src)
rewrite_ids(lr, "r61-ws-dark-lr-")
lr.set("id", "r61-ws-dark-land-raider")
lr.set("name", "Land Raider")
lr.set("hidden", "false")
mods = lr.find(C("modifiers"))
if mods is not None:
    lr.remove(mods)
for r in lr.findall('.//' + C("rule")):
    if (r.get("name") or "").lower() == "land raider dedicated transport":
        r.set("name", "Land Raider")
        d = r.find(C("description"))
        if d is not None:
            d.text = "Select a Land Raider pattern with sufficient Transport Capacity. Use the normal profile, wargear and vehicle-upgrade rules for that pattern."
ensure(dark_transport, "selectionEntryGroups").append(lr)

# Jump Packs and a Dedicated Transport are mutually exclusive in both directions.
transport_max = next((c for c in dark_transport.findall('./' + C("constraints") + '/' + C("constraint")) if c.get("type") == "max"), None)
assert transport_max is not None
add_conditional_modifier(
    dark_transport, "r61-ws-dark-hide-transport-with-jump", "hidden", "true",
    dark_jump.get("id"), "root-entry"
)
add_conditional_modifier(
    dark_transport, "r61-ws-dark-zero-transport-with-jump", transport_max.get("id"), "0",
    dark_jump.get("id"), "root-entry"
)
add_rule(
    dark, "r61-ws-dark-transport-rule", "Dedicated Transport",
    "Without Jump Packs, the unit may select a Rhino, Drop Pod, Dreadclaw Drop Pod or Land Raider where "
    "Transport Capacity permits. If the squad takes Jump Packs, every model must take them and the unit may "
    "not select a Dedicated Transport."
)

# Revision bump: CAT and GST together so New Recruit refreshes the data.
cr.set("revision", "61")
cr.set("gameSystemRevision", "29")
gr.set("revision", "29")
comment = cr.find(C("comment"))
if comment is not None:
    comment.text = "Revision 61: psychic-discipline profiles for Librarians and Stormseers, Unseen Bolt weapon profile, and corrected White Scars transports."
...
assert vals == {
    "Range": '36"', "S": "6", "AP": "4", "Type": "Assault 1, Blast, Pinning, Psychic"
}, vals

direct_targets = sorted(l.get("targetId") for l in dest_links.findall(C("entryLink")))
assert direct_targets == sorted(wanted_targets), direct_targets
lr_check = by_id(cr, "r61-ws-dark-land-raider")
assert lr_check is not None
lr_entries = lr_check.find(C("selectionEntries"))
assert lr_entries is not None
lr_names = sorted(e.get("name") for e in lr_entries.findall(C("selectionEntry")))
assert lr_names == sorted(["Land Raider Phobos", "Land Raider Proteus", "Land Raider Achilles"]), lr_names
assert by_id(cr, "r61-ws-dark-hide-transport-with-jump") is not None
assert by_id(cr, "r61-ws-dark-zero-transport-with-jump") is not None
...
note("Dark Sons transport validation: exact Rhino, Drop Pod and Dreadclaw targets plus Land Raider Phobos/Proteus/Achilles; transport is hidden and capped at zero when Jump Packs are selected.")
