...
g=group(u,'r63-sw-slayer-shields','Bolter Replacements — any model',maxv=20); local_option(g,'r63-sw-slayer-shield','Combat Shield',3,maxv=20,'Replaces that model’s Bolter.')
g=per_five_group(u,'r63-sw-slayer-ranged','Special Weapons — one per five models',mid,[10,15,20])
for k,n,c in [('flamer','Flamer',5),('melta','Meltagun',10),('plasma','Plasma Gun',15)]: local_option(g,'r63-sw-slayer-'+k,n,c,maxv=4,'Replaces one Grey Slayer’s Bolter.')
g=per_five_group(u,'r63-sw-slayer-melee','Melee Weapons — one per five models',mid,[10,15,20])
for k,n,c in [('power','Power Weapon',10),('frost','Frost Blade or Frost Axe',15),('fist','Power Fist',15)]: local_option(g,'r63-sw-slayer-'+k,n,c,maxv=4,'Replaces one Grey Slayer’s Close-combat weapon.')
...
for k,n,c in [('flamer','Flamer',5),('melta','Meltagun',10),('plasma','Plasma Gun',15),('volkite','Volkite Charger',10)]: local_option(g,'r63-sw-stalker-'+k,n,c,maxv=3,'Replaces one Grey Stalker’s Bolter.')
...
for k,n,c in [('bolter','Bolter',0),('shotgun','Astartes Shotgun',0),('sniper','Sniper Rifle',5)]: local_option(g,'r63-sw-scout-'+k,n,c,maxv=10,'Replaces that model’s Bolt pistol and Close-combat weapon.')
...
for k,n,c in [('fist','Power Fist',5),('great','Great Frost Blade',10),('hammer','Thunder Hammer',10)]: local_option(g,'r63-sw-deathsworn-'+k,n,c,maxv=2,'Replaces one Deathsworn’s Power Weapon.')
...
for k,n,c in [('foe','Foeblaster Boltgun',5),('flamer','Combi-flamer',10),('volkite','Combi-volkite charger',10),('melta','Combi-meltagun',15),('plasma','Combi-plasma',15)]: local_option(g,'r63-sw-varagyr-'+k,n,c,maxv=10,'Replaces that model’s Combi-bolter.')
g=group(u,'r63-sw-varagyr-melee','Frost Weapon Replacements — any model',maxv=10)
for k,n,c in [('fist','Power Fist',5),('chain','Chainfist',10),('hammer','Thunder Hammer',10)]: local_option(g,'r63-sw-varagyr-melee-'+k,n,c,maxv=10,'Replaces that model’s Frost Blade or Frost Axe.')
g=per_five_group(u,'r63-sw-varagyr-heavy','Heavy Weapons — one per five models',mid,[10])
for k,n,c in [('hf','Heavy Flamer',10),('reaper','Reaper Autocannon',15),('assault','Assault Cannon',20)]: local_option(g,'r63-sw-varagyr-heavy-'+k,n,c,maxv=2,'Replaces one Varagyr’s Combi-bolter.')
thegn=local_option(u,'r63-sw-varagyr-thegn','Upgrade one Varagyr to a Thegn',25,maxv=1,'The Thegn uses the Thegn profile in this entry.')
